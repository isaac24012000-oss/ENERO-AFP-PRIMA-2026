...
st.set_page_config(layout="wide", page_icon="🏦", page_title="AFP Enero 2026")
...
EXCEL_PATH = os.path.join(os.getcwd(), "DATA TOTAL WORLDTEL ENERO 2026.xlsx")  # Confirmar ruta relativa
...
# CRÍTICO: PRIORIDAD contiene "15" + CONTACTABILIDAD = "Contacto Directo" + TIPO DE PAGO = "FALTA PAGO DE PAGO PLANILLAS"
# CRÍTICO: PRIORIDAD contiene "15" + CONTACTABILIDAD = "Contacto Directo" + TIPO DE PAGO = "FALTA PAGO DE PAGO PLANILLAS"
# CRÍTICO: PRIORIDAD inicia con "15", CONTACTABILIDAD tiene algún valor y REC. PLANILLAS está vacío o cero
cond_critico = (
    df_analisis['PRIORIDAD'].astype(str).str.startswith('15') &
...
# ALTO: PRIORIDAD contiene "15" pero NO es crítico
cond_alto = (
    df_analisis['PRIORIDAD'].astype(str).str.startswith('15') & (~cond_critico)
)
df_analisis.loc[cond_alto, 'NIVEL_RIESGO'] = 'ALTA'
# MEDIO: PRIORIDAD contiene "11", "10", "09", "08", "07", "06", "05"
cond_medio = df_analisis['PRIORIDAD'].astype(str).str.startswith(('13','12','11', '10', '09', '08', '07', '06', '05'))
...
<div style='width:100%; display:flex; justify-content:center; margin:24px 0 12px 0;'>
    <div style='display:flex; gap:38px; align-items:center; background:#f7f9fc; border-radius:16px; padding:18px 32px;'>
        ...
        <span style='font-size:1.1em;'>🧨 <b>+ALTA:</b> Prioridad 15 + Contacto Directo + Sin Pago</span>
        <span style='font-size:1.1em; color:#388e3c;'>🟢 <b>ALTA:</b> Prioridad 15 (todos)</span>
        <span style='font-size:1.1em; color:#fbc02d;'>🟡 <b>MEDIA:</b> Prioridades 5-14</span>
        <span style='font-size:1.1em; color:#d32f2f;'>🔴 <b>BAJA:</b> Prioridades 1-4</span>
    </div>
</div>
""", unsafe_allow_html=True)
...
def export_to_excel(df_export):
    ...
    # Encabezado con título
    ws['A1'] = "CASOS CRÍTICOS - PRIORIDAD 15 + CONTACTO DIRECTO + SIN PAGO"
    ws['A1'].font = Font(bold=True, size=14, color="FFFFFF")
...
<div style='display: flex; align-items: center; margin-top:32px;'>
    ...
    <h2 style='display: inline; font-size: 2rem; margin: 0; color: #c62828;'>
        Detalle de Casos Críticos (Prioridad 15 + Contacto Directo + Sin Pago)
    </h2>
</div>
""", unsafe_allow_html=True)
